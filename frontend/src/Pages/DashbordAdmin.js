...
const fetchMessages = async () => {
    try {
        const res = await axios.get("http://localhost:5000/api/messages/unanswered");
...
const [clients, setClients] = useState([]);
  ...
  const handleDownload = async (fileId, filenamePrefix) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/auth/file/download/${fileId}`, {
        responseType: 'blob',
      });
...
const unansweredMessages = messages.filter(
  ...
  const fetchAgencies = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/auth/admin/all-agencies");
  ...
  const fetchInvestors = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/auth/admin/all-investors");
...
const fetchClients = async () => {
  try {
    const res = await axios.get("http://localhost:5000/api/auth/admin/all-clients");
    if (res.data.success) {
  ...
  const handleRejectAgency = async (id) => {
    try {
      await axios.patch(`http://localhost:5000/api/auth/agency/reject/${id}`);
  ...
  const handleRejectInvestor = async (id) => {
    try {
      await axios.patch(`http://localhost:5000/api/auth/investor/reject/${id}`);
  ...
  const handleVerifyProof = async (userType, id) => {
    try {
      await axios.post(`http://localhost:5000/api/auth/admin/verify-proof/${userType}/${id}`);
...
const handleDeleteAgency = async (id) => {
  if (window.confirm("Are you sure you want to delete this agency?")) {
    try {
      await axios.delete(`http://localhost:5000/api/auth/admin/delete-agency/${id}`);
...
const handleDeleteInvestor = async (id) => {
  if (window.confirm("Are you sure you want to delete this investor?")) {
    try {
      await axios.delete(`http://localhost:5000/api/auth/admin/delete-investor/${id}`);
...
const handleDeleteClient = async (id) => {
  if (window.confirm("Are you sure you want to delete this client?")) {
    try {
      await axios.delete(`http://localhost:5000/api/auth/admin/delete-client/${id}`);
                  ...
                  <div style={{ marginLeft: "20px"}}>
                    {agency.proofFileId && (
                      <>
                          ...
                          <img
                          src={`http://localhost:5000/api/auth/file/view/${agency.proofFileId}`}
                          alt="Proof File"
            ...
            <div key={index} style={{ ...styles.displayBox, display: "flex", justifyContent: "space-between" }}>
              ...
              <div style={{ marginLeft: "20px" }}>
                {investor.proofFileId && (
                  <>
                    <img
                      src={`http://localhost:5000/api/auth/file/view/${investor.proofFileId}`}
                      alt="Proof File"
                      style={{ width: "300px", maxHeight: "200px", cursor: "pointer", marginBottom: "10px" }}
