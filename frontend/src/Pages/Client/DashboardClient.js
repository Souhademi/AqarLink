  ...
  const handleLogout = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/auth/client/logout",
        { clientId },
        {
          headers: {
...
const propertyTypes = ["Apartment", "Villa", "Land", "Garage"];
  ...
  //       `http://localhost:5000/api/auth/client/notifications/${clientId}`,
  ...
  //       await axios.put(
  //         `http://localhost:5000/api/auth/client/notifications/${clientId}/mark-seen`,
  //         { ids: [latest._id] },
  //         { headers: { Authorization: `Bearer ${token}` } }
  //       );
  //     }
